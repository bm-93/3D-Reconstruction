...
def displayEpipolarF(I1, I2, F):
    ...
    closeid = f.canvas.mpl_connect('close_event', closeEvent)
    while True:
        plt.sca(ax1)
        mouseinput = plt.ginput(1, timeout=3600, mouse_stop=2)
...
def epipolarMatchGUI(I1, I2, F):
    ...
    while True:
        ...
        xc = int(x)
        yc = int(y)
        pts1.append(np.append(xc,yc).reshape((2,)))
        v = np.array([xc, yc, 1])
        l = F.dot(v)
        s = np.sqrt(l[0]**2+l[1]**2)
        ...
        pts2.append(np.append(x2,y2).reshape((2,)))
